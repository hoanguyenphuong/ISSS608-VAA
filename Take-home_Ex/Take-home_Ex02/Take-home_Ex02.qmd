...
To answer these two questions, we construct a interactive network graph showing who communicates with whom (among the people and vessels) based on the intercepted radio messages.
...
• Filter: Use the “Select by id” dropdown at the top to highlight an individual node along with all entities it communicates with directly. Use the "Select by cluster" dropdown right below to highlight nodes within a certain cluster.

• Interactivity: Users can zoom in and out, use the four-directional arrow controls to pan across the graph, and click the full-view button to recenter the entire network layout — making it easier to explore dense or scattered communication patterns.
...
# Step 0: Filter only Person and Vessel interactions
edge_list <- comm_full %>%
...
cluster_names <- paste0("Cluster ", 1:5)
cluster_map <- setNames(cluster_names, as.character(1:5))
...
nodes$group <- factor(nodes$group, levels = cluster_names)
...
# Step 6: Render network with full navigation UI and "Select by cluster"
visNetwork(nodes, edges) %>%
  visOptions(
    highlightNearest = TRUE,
    selectedBy = list(variable = "group", multiple = FALSE, main = "Select by cluster"),  # 👈 custom label
...
• The largest nodes by size like Mako and Reef Guardians represent the most active communicators, each with a high total number of messages sent and received. In contrast to these dominant hubs, nodes like Samantha Blake and Osprey participate less frequently, possibly playing supporting roles.
...
The communication network of people and vessels reveals five distinct clusters, each representing a group of entities that communicate more frequently within their group/cluster than with outside entities, suggesting likely shared objectives or roles. The clusters were derived using the Louvain algorithm, which detects tightly connected groups within the network. Each cluster is color-coded for visual clarity.

🔵 Cluster 1 consists of 6 nodes: The Accountant, Boss, the Middleman, Mrs. Money, The Intern, and The Lookout.

🔴 Cluster 2 consist of 14 nodes: Samantha Blake, Osprey, Elise, Knowles, Liam Thorne, Mako, Small Fry, Neptune, Davis, Nadia Conti, Remora, Rodriguez, Marlin, and Serenity.

🟣 Cluster 3 consists of 2 nodes: Sam and Kelly. These two entities barely communicate and, if they do, primarily interact with each other, as their only outside communication is with Cluster 1.

🟢 Cluster 4 consists of 6 nodes: EcoVigil, Defender, Reef Guardian, Seawatch, Sentinel, and Horizon.
...
As we can see, the people and vessels network structure is well-organized: most entities belong to well-defined clusters with strong internal communication ties.
...
comm_with_cluster <- comm_full %>%
  filter(sender_type %in% c("Person", "Vessel"),
         receiver_type %in% c("Person", "Vessel")) %>%  # Filter to only Person/Vessel
...
# Step 5: Get top 20 keywords per cluster
...
# Preview example: View top 20 for Cluster 1
...
# Preview: View top 20 for Cluster 4
...
# Preview: View top 20 for Cluster 5
...
comm_with_cluster <- comm_full %>%
  filter(sender_type %in% c("Person", "Vessel"),
         receiver_type %in% c("Person", "Vessel")) %>%  # ✅ Only People and Vessels
  left_join(node_cluster, by = c("sender_label" = "label")) %>%
  rename(sender_cluster = cluster) %>%
  left_join(node_cluster, by = c("receiver_label" = "label")) %>%
  rename(receiver_cluster = cluster)
...
🔵 Cluster 1 shows signs of being a coordinating or facilitative cluster. Top words like “meeting”, "tomorrow", “birdwatching”, and "discuss" suggest involvement in administrative work, strategic planning, or cross-group information flow.

🔴 Cluster 2 appears to be focused on maritime operations. Keywords like “equipment”, "harbor", "maintain", and "operation(s)" suggest activity around marine logistics, vessel coordination, and technical operations — likely indicating a team involved in sea-based field operations or enforcement.

🟣 Cluster 3 is characterized by limited communication and does not contain any keywords of significant frequency. This lack of prominent terms makes it difficult to infer any clear thematic focus for the group.

🟢 Cluster 4 centers around spatial coordination, regulation, and oversight. Frequently used terms such as "guardian", "sentinel", "activity", "position", and "boundary" suggest a group concerned with marine surveillance, monitoring zones, and geographic boundaries.

🟡 Cluster 5 shows slightly more communication and includes certain frequently used terms than Cluster 3. However, its complete isolation from the rest of the network limits contextual interpretation and suggests a potentially detached or non-integrated role.
...
# Step 0: Filter only communications between people and vessels
comm_filtered <- comm_full %>%
  filter(sender_type %in% c("Person", "Vessel"),
         receiver_type %in% c("Person", "Vessel"))
...
# Step 2: Extract all mentions of pseudonym terms from filtered comm
pseudonym_mentions <- comm_filtered %>%
...
The first chart displays how often each suspected pseudonym appears in the intercepted messages. Names like “Mako”, “Neptune”, and “Remora” are mentioned with unusually high frequency. Not only do they appear far more often than most other entities, but their naming style also deviates from conventional personal or organizational names — raising strong suspicions that they may be aliases. Names such as “Mrs. Money”, “The Lookout”, “Boss”, and “The Intern” appear less frequently than the top three, but still much more than the remainings. Their unusual phrasing — resembling roles or nicknames rather than real names — further supports earlier suspicions (including Clepper’s) that they may be pseudonyms used informally in communication. A few other names such as “The Middleman”, “Serenity”, and “Marlin”, are mentioned less often but still draw attention due to their nonstandard naming format. Despite lower frequency, their distinctiveness suggests they could also serve as aliases.

The second visualisation, a heatmap, reveals who is mentioning each pseudonym:

• “Mako” is mentioned heavily by “Davis”, followed by “Remora” and “Neptune”.

• “Neptune” is used most frequently by “Remora”, followed closely by “Mako”.

• “Remora” is most often mentioned by “Neptune”, with “Mako” and "Rodriguez" being the next most frequent users.

• “Mrs. Money” is mentioned heavily by “The Intern”, followed by "The Middleman" though at lower frequencies.

• “The Lookout” is heavily referenced by and only by “The Intern”.

• “Boss” is frequently mentioned by “The Middleman”, followed by “Mrs. Money”.

• “The Intern” is used most by “The Lookout”.

• “The Middleman” is mainly mentioned by "Mrs. Money" and "The Accountant".

• “Serenity” is referenced primarily by “Reef Guardian”.

• “Marlin” is mentioned most often by “Reef Guardian" and "Davis".
